...
describe("publisher-013-01 : Mock the api response and test it", () => {
  ...
  const createProductAndUpdateUnderlineApiSpec = (tenant) => {
    ...
    cy.get("#itest-id-apiversion-input", {
      timeout: Cypress.config().largeTimeout,
    });
    cy.document().then((doc) => {
      ...
      // finish the wizard
      cy.get("#open-api-create-btn")
        .should("not.have.class", "Mui-disabled")
        .click({ force: true });

      cy.url({ timeout: Cypress.config().largeTimeout })
        .should("contains", "overview")
        .then((url) => {
          ...
          cy.wait("@swaggerGet", {
            ...
            cy.wait("@getSwagger", { timeout: Cypress.config().largeTimeout });
            ...
            cy.intercept("POST", "**/api-products").as("getapiproducts");
            cy.get("#create-api-product-btn")
              .scrollIntoView()
              .dblclick({ force: true });
            cy.wait("@getapiproducts", {
              timeout: Cypress.config().largeTimeout,
            });
            cy.url({ timeout: Cypress.config().largeTimeout })
              .should("contains", "overview")
              .then((urlProduct) => {
                const productID = /api-products\/(.*?)\/overview/.exec(
                  urlProduct
                )[1];
                ...
                cy.visit(`/publisher/apis/${testApiID}/resources`, { timeout: Cypress.config().pageLoadTimeout });

                // Add a new resource to the underline api
                // Typing the resource name
                const target = "/test";
                cy.get("#operation-target", {
                  timeout: Cypress.config().largeTimeout,
                });
                ...
                cy.get("#resource-wrapper", {timeout: 20000}).last().scrollIntoView().click();
  ...
  it(
    "Mock the api response and test it - super admin",
    {
      retries: {
        runMode: 3,
        openMode: 0,
  ...
  it(
    "Mock the api response and test it - tenant user",
    {
      retries: {
        runMode: 3,
        openMode: 0,
    ...
    () => {
      createProductAndUpdateUnderlineApiSpec(testTenant);
    }
  );
  afterEach(() => {
