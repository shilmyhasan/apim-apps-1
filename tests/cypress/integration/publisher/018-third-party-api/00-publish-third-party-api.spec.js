...
describe("publisher-018-00 : Publish thirdparty api", () => {
    ...
    const publishThirdPartyApi = (tenant) => {
        ...
        publisherComonPage.waitUntillPublisherLoadingSpinnerExit();
            ...
            // publisherComonPage.waitUntillPublisherLoadingSpinnerExit();
            // cy.get('#outlined-full-width', {timeout: Cypress.config().largeTimeout})
            // .invoke('val', 'https://raw.githubusercontent.com/asyncapi/spec/v2.0.0/examples/2.0.0/streetlights.ym');
            // cy.get('#outlined-full-width').type('l');
            // cy.get('#outlined-full-width').click(0,0);
            // cy.get('#outlined-full-width')
            //.should('have.value','https://raw.githubusercontent.com/asyncapi/spec/v2.0.0/examples/2.0.0/streetlights.yml');
            ...
            cy.url().then(url => {
                ...
                cy.get(`div[data-testid="card-action-${apiName}1.0.0"]>div>span`,
                 {timeout: Cypress.config().largeTimeout}).contains('PUBLISHED').should('exist');
    ...
    it.only("Publish thirdparty api - super admin", {
        retries: {
            runMode: 3,
            openMode: 0,
        },
    }, () => {
        publishThirdPartyApi(superTenant);
    });
});
